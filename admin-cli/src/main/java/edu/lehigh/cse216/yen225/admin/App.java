...
public class App {
    ...
    public static void menu() {
        ...
        System.out.println("  [V] Validate a user, post, comment");
        System.out.println("  [I] Invaildate a user, post, comment");
        System.out.println("  [X] Delete all invalid users, posts, and comments");
            ...
            if (action == '?') {
              ...
              //  ArrayList<Database.RowDataUser> resUser = db.selectAllUser();
                ...
                for (Database.RowData rd : res) {
                    ...
                    System.out.println("  [" + rdUser.mUserID + "] " + rdUser.mUsername + ":" + rdUser.mEmail + ":" + rdUser.mGenderID + ":" + rdUser.mSexO + ":"+ rdUser.mBio + ":" + rdUser.mValidity);
                    }
                }
            } else if (action == '-') {
                int id = getInt(in, "Enter the row ID");
                ...
                if(tableName.equals("tblUser")){
                    ...
                    db = Database.getDatabase(ip, port, user, pass,tableName);
                    db.validate(tbl, id);
                } else if(tableName.equals("tblData")){
                    String tbl = tableName;
                    ...
                    db = Database.getDatabase(ip, port, user, pass,tableName);
                    db.validate(tbl, id);
                }else if(tableName.equals("tblComment")){
                    String tbl = tableName;
                ...
                if(tableName.equals("tblUser")){
                    tableName = "tblData";
                    db = Database.getDatabase(ip, port, user, pass, tableName);
                    res = db.deleteInvalid();
                    System.out.println("  " + res + " posts deleted");
                    tableName = "tblComment";
                    db = Database.getDatabase(ip, port, user, pass, tableName);
                    res = db.deleteInvalid();
